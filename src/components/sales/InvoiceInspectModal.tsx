    ...
    <Modal isOpen={Boolean(sale)} onClose={onClose} title={`Invoice ${sale.invoiceNo}`} description="Itemized receipt and payment ledger record" icon={<Receipt className="size-5 text-brand-500" />} size="lg">
      <div className="space-y-4 text-xs">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                ...
                <tr key={it.id}>
                  ...
                  <td className="p-2.5 text-center tabular-nums">{it.quantity}</td>
                  <td className="p-2.5 tabular-nums">PKR {Number(it.unitPrice).toLocaleString()}</td>
                  <td className="p-2.5 text-right font-bold tabular-nums">PKR {Number(it.totalPrice).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="p-3 rounded-xl bg-brand-50/60 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 space-y-1">
          <div className="flex justify-between"><span>Subtotal:</span><span className="tabular-nums font-semibold">PKR {Number(sale.subtotal || 0).toLocaleString()}</span></div>
          {Number(sale.discount || 0) > 0 && <div className="flex justify-between text-success-600"><span>Discount:</span><span className="tabular-nums font-semibold">-PKR {Number(sale.discount).toLocaleString()}</span></div>}
          <div className="flex justify-between font-bold text-sm text-brand-700 dark:text-brand-300 pt-1 border-t border-brand-200 dark:border-brand-500/30">
            <span>Total Invoiced:</span><span className="tabular-nums">PKR {Number(sale.totalAmount || 0).toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-gray-600 dark:text-gray-300"><span>Amount Paid:</span><span className="tabular-nums">PKR {Number(sale.paidAmount || 0).toLocaleString()}</span></div>
          {Number(sale.balanceDue || 0) > 0 && <div className="flex justify-between font-bold text-error-600"><span>Balance Due:</span><span className="tabular-nums">PKR {Number(sale.balanceDue).toLocaleString()}</span></div>}
        </div>
        ...
        <div className="flex justify-end items-center gap-2.5 pt-2">
          ...
          <button onClick={() => onPrintReceipt(sale)} className="tail-btn-primary-sm flex items-center gap-1.5"><Printer className="size-4" /><span>Print Receipt</span></button>
        </div>
      </div>
    </Modal>
  );
